...
namespace Retail.Controls
{
    ...
    public partial class Calendar : ContentView
    {
        ...
        public static readonly BindableProperty SwipeLeftCommandProperty =
          BindableProperty.Create(nameof(SwipeLeftCommand), typeof(ICommand), typeof(Calendar), null,BindingMode.TwoWay, propertyChanged: SwipeleftEvent);

        private static void SwipeleftEvent(BindableObject bindable, object oldValue, object newValue)
        {
        ...
        public static readonly BindableProperty SwipeRightCommandProperty =
          BindableProperty.Create(nameof(SwipeRightCommand), typeof(ICommand), typeof(Calendar), null, BindingMode.TwoWay, propertyChanged: SwipeRightEvent);

        private static void SwipeRightEvent(BindableObject bindable, object oldValue, object newValue)
        {
        ...
        private void ChangeDisplayedMonth(int monthsToAdd)
        {
            ...
            if (monthsToAdd == (-1))
            {
                if (BindingContext is Retail.ViewModels.Attendance.PromoterAttendanceViewModel dayModel)
                {
                    dayModel.SwipeLeftCommand?.Execute(targetDisplayedMonth);
                }
                else if (BindingContext is Retail.ViewModels.Attendance.AttendanceSummariesViewModel AttendanceSummariesViewModel)
                {
                    AttendanceSummariesViewModel.SuperSwipeLeftCommand?.Execute(targetDisplayedMonth);
                }
            }
            else
            {
                if (BindingContext is Retail.ViewModels.Attendance.PromoterAttendanceViewModel dayModel)
                {
                    dayModel.SwipeRightCommand?.Execute(targetDisplayedMonth);
                }
                else if (BindingContext is Retail.ViewModels.Attendance.AttendanceSummariesViewModel AttendanceSummariesViewModel)
                {
                    AttendanceSummariesViewModel.SuperSwipeRightCommand?.Execute(targetDisplayedMonth);
                }
            }
